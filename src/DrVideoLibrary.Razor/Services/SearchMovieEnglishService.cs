...
internal class SearchMovieEnglishService : ISearchMovieService<SearchMovieEnglishService>
{
    ...
    public SearchMovieEnglishService(HttpClient client, IOptions<SearchMovieOptions> options, ITranslateService translationService)
    {
    ...
    {
        HttpResponseMessage response = await Client.GetAsync($"?i={id}&apikey={Options.ApiKey}");
    ...
    {
        HttpResponseMessage response = await Client.GetAsync($"?s={text}&apikey={Options.ApiKey}");
        response.EnsureSuccessStatusCode();
