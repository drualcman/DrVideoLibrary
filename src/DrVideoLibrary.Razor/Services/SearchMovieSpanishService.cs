...
internal class SearchMovieSpanishService : ISearchMovieService<SearchMovieSpanishService>
{
    ...
    {
        HttpResponseMessage response = await Client.GetAsync($"movie/{id}?api_key={Options.ApiKey}&language=es-ES&append_to_response=credits");
    ...
    {
        HttpResponseMessage response = await Client.GetAsync($"search/movie?api_key={Options.ApiKey}&query={text}&language=es-ES");
        response.EnsureSuccessStatusCode();
