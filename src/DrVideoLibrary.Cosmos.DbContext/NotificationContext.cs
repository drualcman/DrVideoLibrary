...
internal class NotificationContext : INotificationContext
{
    ...
    {
        await GetContainer().UpsertItemAsync(ObjectConverter.ConvertToLowercaseObject(subscription, "subscriptions"), Subscriptions);
    }
    ...
    public async Task<IEnumerable<SubscriptionModel>> GetSubscriptions()
    {
        ...
        FeedIterator<SubscriptionModel> query = GetContainer().GetItemQueryIterator<SubscriptionModel>(new QueryDefinition(queryString));
